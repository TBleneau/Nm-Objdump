...
static unsigned char elf32_type_sections(size_t flags)
{
...
static unsigned char elf32_type_flags(Elf32_Sym symbol, Elf32_Shdr *elf_S)
{
	unsigned char type = '?';
	...
	if (sect_type == SHT_PROGBITS)
		type = elf32_type_sections(flags);
	if (flags == SHF_ALLOC)
		type = 'R';
...
static unsigned char elf32_type_index(Elf32_Sym symbol, Elf32_Shdr *elf_S)
{
	unsigned char type = '?';
	...
	case SHT_DYNAMIC:
		...
		break;
	default:
		type = elf32_type_flags(symbol, elf_S);
		break;
	}
	return (type);
}

static unsigned char elf32_type_weak(Elf32_Sym symbol)
{
...
unsigned char elf32_types(Elf32_Sym symbol, Elf32_Shdr *elf_S)
{
	unsigned char type = '?';
	size_t elf_bind = ELF32_ST_BIND(symbol.st_info);
	...
	case STB_GNU_UNIQUE:
		...
		break;
	case STB_WEAK:
		type = elf32_type_weak(symbol);
		break;
	default:
		type = elf32_type_index(symbol, elf_S);
		break;
	}
