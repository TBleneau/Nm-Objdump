...
static int list_add_symbols(elf_sym_t **elf_sym, Elf64_Addr const value,
...
elf_sym_t *elf64_symbols(Elf64_Sym *symbls, size_t index,
				char *str_tab, Elf64_Shdr *elf_S)
{
	int ret = 0;
	size_t inc = 0;
	unsigned char symbol_t = '?';
	elf_sym_t *elf_sym = NULL;
	size_t size = (elf_S + index)->sh_size / sizeof(Elf64_Sym);

	while (inc < size) {
		if ((str_tab + symbls[inc].st_name)[0] != '\0'
			&& ELF64_ST_TYPE(symbls[inc].st_info) != STT_FILE) {
			symbol_t = elf64_types(symbls[inc], elf_S);
