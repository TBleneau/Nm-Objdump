...
int elf32_content_value(size_t *index, size_t const number,
...
unsigned char *elf32_content_init(elf_Ehdr_t const *const elf_Ehdr,
				size_t const index, size_t const size)
{
	unsigned char *cont = calloc(SECT_CONTENT + 1, sizeof(unsigned char));
	unsigned int reference = 0;
	size_t size_sect = elf_Ehdr->elfS_32[index].sh_size;
	size_t pos = (size_sect % 16 == 0) ? 16 : size_sect % 16;
	size_t hexa = elf_Ehdr->elfS_32[index].sh_addr + (size_sect - pos);
	...
	reference <= 4 ? printf(" %04lx ",
			elf_Ehdr->elfS_32[index].sh_addr + size) :
		printf(" %0*lx ", reference,
			elf_Ehdr->elfS_32[index].sh_addr + size);
	return (cont);
}

int elf32_content_update(unsigned char *content,
